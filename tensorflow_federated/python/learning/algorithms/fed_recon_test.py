...
class TrainingProcessTest(tf.test.TestCase, parameterized.TestCase):
  ...
  ) -> tuple[composers.LearningAlgorithmState, list[collections.OrderedDict[
      str, Any]], composers.LearningAlgorithmState]:
  ...
  def test_build_train_iterative_process(self, optimizer_fn):
    ...
    self.assertIsInstance(learning_process,
  ...
  def test_server_update_with_inf_weight_is_noop(self):
    ...
    state, _, initial_state = self._run_rounds(learning_process, federated_data,
                                               1)
    self.assertAllClose(state.global_model_weights.trainable,
                        initial_state.global_model_weights.trainable, 1e-8)
    self.assertAllClose(state.global_model_weights.trainable,
                        initial_state.global_model_weights.trainable, 1e-8)
  ...
  def test_keras_global_model(self, optimizer_fn):
    ...
    self.assertNotAllClose(server_states[0].global_model_weights.trainable,
    ...
    self.assertCountEqual(metrics[0]['client_work']['train'].keys(),
                          expected_train_keys)
  ...
  def test_keras_local_layer(self, optimizer_fn):
    ...
    self.assertCountEqual(output['aggregator'].keys(),
  ...
  def test_keras_local_layer_client_weighting_enum_num_examples(self):
    ...
    self.assertCountEqual(metrics['client_work']['train'].keys(),
                          expected_train_keys)
    ...
    expected_aggregation_keys = ['mean_weight', 'mean_value']
    self.assertCountEqual(metrics['aggregator'].keys(),
                          expected_aggregation_keys)
  ...
  def test_keras_local_layer_metrics_empty_list(self):
    ...
    self.assertCountEqual(metrics['client_work'].keys(),
                          expected_client_work_keys)
  ...
  def test_keras_local_layer_metrics_none(self):
    ...
    expected_client_work_keys = ['train']
    self.assertCountEqual(output.metrics['client_work'].keys(),
                          expected_client_work_keys)
    expected_train_keys = ['loss']
    self.assertCountEqual(output.metrics['client_work']['train'].keys(),
                          expected_train_keys)
  ...
  def test_custom_model_no_recon(self, optimizer_fn):
    ...
    train_metrics = (metrics[0]['client_work']['train'],
    ...
    self.assertNotAllClose(states[0].global_model_weights.trainable,
  ...
  def test_keras_local_layer_custom_broadcaster(self):
    ...
    def build_custom_stateful_distributor(
        model_weights_type) -> distributors.DistributionProcess:
