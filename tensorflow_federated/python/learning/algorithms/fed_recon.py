...
from tensorflow_federated.python.learning.metrics import finalizer as metrics_finalizers_lib
...
def _build_reconstruction_client_work(
  ...
  metric_finalizers: collections.OrderedDict[
      str,
      metrics_finalizers_lib.KerasMetricFinalizer] = collections.OrderedDict()
  ...
  def client_update(initial_model_weights, dataset):
    ...
    with tf.init_scope():
      ...
      if metrics_fn is None:
        metrics = [ReconstructionModel.MeanLossMetric(loss_fn())]
      else:
        metrics = metrics_fn() + [ReconstructionModel.MeanLossMetric(loss_fn())]
      ...
      for metric in metrics:
        ...
        metric_finalizers[
            metric.name] = metrics_finalizers_lib.create_keras_metric_finalizer(
                metric)
    ...
    # The finalizer componenet expects model updates computed as `initial -
    # final`.
    ...
    return client_works.ClientResult(weights_delta,
                                     client_weight), unfinalized_metrics
...
def build_fed_recon(
    ...
    *,  # Callers pass below args by name.
    ...
    metrics_aggregator: Optional[Callable[
        [MetricFinalizersType, computation_types.StructWithPythonType],
        computation_base.Computation]] = metrics_aggregators.sum_then_finalize,
  ...
  py_typecheck.check_type(model_aggregator_factory,
