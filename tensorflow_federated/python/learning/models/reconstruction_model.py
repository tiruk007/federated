...
class ReconstructionBatchOutput:
...
class ReconstructionModel(metaclass=abc.ABCMeta):
  ...
  def get_global_variables(
      cls, model: 'ReconstructionModel') -> model_weights.ModelWeights:
  ...
  def has_only_global_variables(cls, model: 'ReconstructionModel') -> bool:
    ...
    del cls  # Unused.
    return (bool(model.local_trainable_variables) or
            bool(model.local_non_trainable_variables))
  ...
  DatasetSplitFn = Callable[[tf.data.Dataset, tf.Tensor],
  ...
  @classmethod
  def build_dataset_split_fn(cls,
                             recon_epochs: int = 1,
    ...
    variables,
    and during evaluation, it refers to calculation of metrics using
    reconstructed
  ...
  def read_metric_variables(
    ...
    for metric in metrics:
      if metric.name in metric_variables:
        raise ValueError(f'Duplicate metric name detected: {metric.name}. '
  ...
  def from_keras_model(
    ...
    if not isinstance(keras_model, tf.keras.Model):
      raise TypeError('Expected `keras_model` to be type `tf.keras.Model`, '
...
class _KerasReconstructionModel(ReconstructionModel):
  ...
  def forward_pass(self,
                   batch_input,
                   training=True) -> ReconstructionBatchOutput:
